...
import io.github.sigmaol.bigdata.bean.RegressBean
import io.github.sigmaol.bigdata.enums.ErrorMsg
import io.github.sigmaol.help.DateHelper
          ...
          case Some(regressSample) =>
            ...
            data.filter(x => regressSample.value.contains(selectKeyFunc(x)))
              ...
              x =>
                ...
                (key, value.filter(x => DateHelper.daysDiff(selectTimeFunc(x), loadDay) < 0))
