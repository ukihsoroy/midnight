 ...
 */
@UtilityClass
public class ReflectHelper extends ReflectionUtils {
	...
	public static PropertyDescriptor[] getPropertiesHelper(Class type, boolean read, boolean write) {
		try {
			PropertyDescriptor[] all = BeanHelper.getPropertyDescriptors(type);
			if (read && write) {
				return all;
			} else {
	...
	public static Property getProperty(Class<?> propertyType, String propertyName) {
		PropertyDescriptor propertyDescriptor = BeanHelper.getPropertyDescriptor(propertyType, propertyName);
		if (propertyDescriptor == null) {
			return null;
		}
		return ReflectHelper.getProperty(propertyType, propertyDescriptor, propertyName);
	}
	...
	public static TypeDescriptor getTypeDescriptor(Class<?> propertyType, String propertyName) {
		Property property = ReflectHelper.getProperty(propertyType, propertyName);
		if (property == null) {
			return null;
		}
	...
	public static <T extends Annotation> T getAnnotation(Class<?> clazz, String fieldName, Class<T> annotationClass) {
		Field field = ReflectHelper.getField(clazz, fieldName);
		if (field == null) {
			return null;
		}
